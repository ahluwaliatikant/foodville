...
class SearchFoodCourt extends StatefulWidget {
  @override
  _SearchFoodCourtState createState() => _SearchFoodCourtState();
}

class _SearchFoodCourtState extends State<SearchFoodCourt> {
  var queryResultSet = [];
  var tempSearchStore = [];
  ...
  initiateSearch(value) {
    ...
    var capitalizedValue = value.substring(0, 1).toString().toUpperCase() + value.substring(1).toString();
    print(capitalizedValue);

    if (queryResultSet.length == 0 && value.length == 1) {
      //the user has typed in only single character
      SearchService().searchByName(capitalizedValue).then((QuerySnapshot snapshot) {
...
//      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: "Search A Food Court".text.make(),
        centerTitle: true,
                ...
                return InkWell(
                  onTap: () {
                    Navigator.of(context).push(MaterialPageRoute(
                        builder: (context) =>
                            RestaurantLogin(tempSearchStore[index]['name'])));
                  },
                  child: ListTile(
